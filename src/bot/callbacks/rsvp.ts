...
export function registerRsvp(bot: Bot<MyContext>) {
  ...
  bot.callbackQuery(/^go:(.+)$/, async (ctx) => {
    ...
    if (event.maxParticipants && goingCount >= event.maxParticipants) {
      await ctx.answerCallbackQuery(`Мест нет 😔 Все ${event.maxParticipants} мест заняты.`);
      return;
    }
    ...
    try {
      await ctx.editMessageText(formatEventCard(updatedEvent!), { reply_markup: keyboard });
    } catch (_) { /* unchanged */ }
