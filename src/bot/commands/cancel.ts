...
export function registerCancel(bot: Bot<Context>) {
  ...
  bot.callbackQuery(/^cancel_confirm:(.+)$/, async (ctx) => {
    ...
    if (!event) { await ctx.answerCallbackQuery('Не найдено.'); return; }
    ...
    if (event.messageId) {
      try { await ctx.api.editMessageText(event.groupId, event.messageId, `❌ ОТМЕНЕНО\n\n🏃 ${event.title}`); } catch (_) {}
    }
    await ctx.editMessageText(`✅ Тренировка «${event.title}» отменена.`);
    await ctx.answerCallbackQuery('Отменено.');
    await ctx.api.sendMessage(event.groupId, `❌ Тренировка «${event.title}» отменена.`);
  });
